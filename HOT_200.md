...
class Solution:
    def longestConsecutive(self, nums: List[int]) -> int:
        ...
        best = 0
        for x in st:
            if x - 1 not in st:  # 关键剪枝，只从“连续段起点”开始扩展
                y = x
                while y in st:
                    y += 1
...
class Solution:
    def removeElement(self, nums: List[int], val: int) -> int:
        left = 0  # nums[0:left] 存放所有不等于 val 的元素
        for num in nums:
            if num != val:
                nums[left] = num
                left += 1
...
class Solution:
    def isValidBST(self, root: Optional["TreeNode"]) -> bool:
        def dfs(node: Optional["TreeNode"], low: float, high: float) -> bool:
            if node is None:
                return True
            if not (low < node.val < high):  # 必须严格小于/大于
                return False
            return dfs(node.left, low, node.val) and dfs(node.right, node.val, high)

        return dfs(root, -inf, inf)  # float("-inf")
```
...
class Solution:
    def pathSum(self, root: Optional["TreeNode"], targetSum: int) -> int:
        def count_from(node: Optional["TreeNode"], target: int) -> int:
            ...
            if node is None:
                return 0
            ans = 1 if node.val == target else 0
            ans += count_from(node.left, target - node.val)
            ans += count_from(node.right, target - node.val)
            return ans
...
class Solution:
    def pathSum(self, root: Optional["TreeNode"], targetSum: int) -> int:
        ...
        def dfs(node: Optional["TreeNode"], cur_sum: int) -> int:
            ...
            ans += dfs(node.left, cur_sum)  # 继续递归左右子树，累加结果
...
class Solution:
    def combinationSum3(self, k: int, n: int) -> List[List[int]]:
        ...
        def dfs(i: int, remain: int) -> None:
            if len(path) == k:  # 选满 k 个数后，不管 remain 是不是 0，都不能继续递归了
                if remain == 0:
                    res.append(path[:])
                return
            
            if 9 - i + 1 < k - len(path):  # 剪枝：剩余数字不足
                return
            
            for j in range(i, 10):  # 只用 1~9
                if j > remain:  # 剪枝：等价于 remain < 0
                    break
                path.append(j)
...
class Solution:
    def find132pattern(self, nums: List[int]) -> bool:
        n = len(nums)
        stack = []  # 单调递减栈，维护 nums[j] 值的候选
        kMax = -inf  # 用于记录出栈元素的最大值（即 nums[k]）
        for i in range(n - 1, -1, -1):  # 倒序遍历 i
            if nums[i] < kMax:  # 如果当前 nums[i] < kMax，说明找到了有效的 (i, k)
                return True
            while stack and stack[-1] < nums[i]:  # 维护单调栈，弹出比 nums[i] 小的元素，找到了有效的 (j, k)
                kMax = max(kMax, stack.pop())  # 事实上，kMax 的变化也具有单调性，可写为 kMax = d.pop()
            stack.append(nums[i])  # 当前 nums[i] 入栈，可能成为后续 k 的候选
        return False
```
...
class Solution:
    def largestRectangleArea(self, heights: List[int]) -> int:
        stack = [-1]  # 单调递增栈：存下标，保证 heights[stack] 递增
        ans = 0
        for i, h in enumerate(heights + [0]):
            # 遇到更矮的柱子时，说明栈顶柱子的“右边第一个更矮位置”已确定，可以确定当前高度的最大宽度
            while stack[-1] != -1 and h < heights[stack[-1]]:
                height = heights[stack.pop()]
                width = (i-1) - (stack[-1]+1) + 1  # 左边第一个更矮是新的栈顶stack[-1]，右边第一个更矮是当前下标i
...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        dp[0][1] = -inf  # 若第0天未持有股票，最大利润为0；第0天不可能持有股票，最大利润为负无穷
...
class Solution:
    def findContentChildren(self, g: List[int], s: List[int]) -> int:
        ...
        m, n = len(g), len(s)
        left = 0  # 已满足的孩子数量
        for right in range(n):  # 遍历饼干
            if left < m and s[right] >= g[left]:  # 当前饼干可以满足第 left 个孩子
                left += 1  # 指向下一个最小需求的孩子
        return left
```
...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        for ch in tasks:
            cnt[ord(ch) - ord('A')] += 1  # ord('A') == 65
        
        max_count = max(cnt)  # 找出最大出现次数
        k = cnt.count(max_count)  # 统计有多少种任务的出现次数等于最大值
