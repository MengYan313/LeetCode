...
class Solution:
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for right, ch in enumerate(s):
            while ch in window:  # 如果有重复字符，就不断收缩左边界
...
class Solution:
    def find132pattern(self, nums: List[int]) -> bool:
        n = len(nums)
        s = []  # 单调递减栈，维护 nums[j] 值的候选
        kMax = -inf  # 用于记录出栈元素的最大值（即 nums[k]）
        for i in range(n - 1, -1, -1):  # 倒序遍历 i
            if nums[i] < kMax:  # 如果当前 nums[i] < kMax，说明找到了有效的 (i, k)
                return True
            while s and s[-1] < nums[i]:  # 维护单调栈，弹出比 nums[i] 小的元素，找到了有效的 (j, k)
                kMax = max(kMax, s.pop())  # 事实上，kMax 的变化也具有单调性，可写为 kMax = d.pop()
            s.append(nums[i])  # 当前 nums[i] 入栈，可能成为后续 k 的候选
        return False
```
